...
int n, m, count, max_count, c, max_color, k;
...
void reset_matrix(int m, int n, vector<vector<int>>& grid)
{
    for(int i = 0; i < m; i++)
        for(int j = 0; j < n; j++)
...
int main()
{
    ...
    cout << largest_contiguous_component(m, n, grid) << '\n';
    return 0;
}